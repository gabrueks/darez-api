...
describe('DELETE /v1/users/addresses', () => {
  ...
  it('should delete an user address', async () => {
    ...
    const [[infoBefore]] = await connection.promise().query('SELECT active FROM user_addresses WHERE id=54321');

    const response = await request(server).delete('/v1/users/addresses').set('Authorization', login.body.access_token).send({
      addresses: [54321],
    });

    const [[infoAfter]] = await connection.promise().query('SELECT active FROM user_addresses WHERE id=54321');

    expect(response.status).toEqual(204);
    expect(infoBefore.active).toBeTruthy();
    expect(infoAfter.active).toBeFalsy();
  });
  ...
  it('should return unauthorized when request without token', async () => {
    const response = await request(server).delete('/v1/users/addresses').send();
