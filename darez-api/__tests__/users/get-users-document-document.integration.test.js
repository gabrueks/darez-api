...
describe('GET /v1/users/document/:DOCUMENT', () => {
  let server;
  let connection;
  ...
  it('should get user information given a document and return it', async () => {
    ...
    const response = await request(server).get('/v1/users/document/12345678955').set('Authorization', login.body.access_token).send();
  ...
  it('should return unauthorized when request without token', async () => {
    const response = await request(server).get('/v1/users/document/12345678955').send();
