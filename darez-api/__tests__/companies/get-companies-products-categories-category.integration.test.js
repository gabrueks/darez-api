...
describe('GET /v1/companies/products/categories/:category', () => {
  let server;
  let connection;
  ...
  it('should get all company products infos from specific category when loged in as company', async () => {
    ...
    const response = await request(server).get('/v1/companies/products/categories/CategoryTwo')
    ...
    expect(response.body.products[0]).toHaveProperty('photo_key');
    expect(response.body.products[0].promotion_id).toEqual(null);
    expect(response.body.bucket_url).toEqual('bucketUrl');
  });

  it('should get all company products infos from specific category', async () => {
    const response = await request(server).get('/v1/companies/products/categories/CategoryTwo')
      .query({ ID: 123987 }).send();
  ...
  it('should return bad request when does not send query params', async () => {
    const response = await request(server).get('/v1/companies/products/categories/CategoryTwo').send();
