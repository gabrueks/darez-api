...
describe('DELETE /v1/admin/companies/:ID/images/logo', () => {
  let server;
  let connection;
  ...
  it('should return status 204 for a company that does not has logo when admin', async () => {
    ...
    const response = await request(server).delete('/v1/admin/companies/123987/images/logo')
  ...
  it('should delete a logo from a company that has a logo when admin', async () => {
    await request(server).post('/v1/users').send({
      phone_country_code: adminPhoneCountryCode,
      phone_area_code: adminPhoneAreaCode,
      phone_number: adminPhoneNumber,
    ...
    const response = await request(server).delete('/v1/admin/companies/54321/images/logo')
      .set('Authorization', login.body.access_token).send();
  ...
  it('should return unauthorized when request without token', async () => {
    const response = await request(server).delete('/v1/admin/companies/54321/images/logo').send();
