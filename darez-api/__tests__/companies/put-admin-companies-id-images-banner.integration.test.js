...
describe('PUT /v1/admin/companies/:ID/images/banner', () => {
  let server;
  let connection;
  ...
  it('should update a company banner without resizing when admin', async () => {
    ...
    const response = await request(server).put('/v1/admin/companies/54321/images/banner')
  ...
  it('should update a company banner after resizing when admin user', async () => {
    const mockSave = Jimp.read.mockImplementation(() => ({
      bitmap: {
        height: 301,
        width: 1281,
  ...
  it('should return unauthorized when request without token', async () => {
    const response = await request(server).put('/v1/admin/companies/54321/images/banner').send();
