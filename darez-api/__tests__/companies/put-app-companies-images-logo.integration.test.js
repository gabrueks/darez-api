...
describe('PUT /v1/app/companies/images/logo', () => {
  let server;
  let connection;
  ...
  it('should update a company logo without resizing', async () => {
    ...
    const [[infoBefore]] = await connection.promise().query('SELECT logo FROM companies WHERE id=54321');

    const response = await request(server).put('/v1/app/companies/images/logo')
      .set('Authorization', login.body.access_token)
      .send({
        file: 'U29tZSBkYXRh', // Base 64 to 'Some data'
        type: '.png',
      });

    const [[infoAfter]] = await connection.promise().query('SELECT logo FROM companies WHERE id=54321');
  ...
  it('should update a company logo after resizing', async () => {
    const mockSave = Jimp.read.mockImplementation(() => ({
      bitmap: {
        height: 301,
        width: 1281,
  ...
  it('should return status of bad request when missing argument', async () => {
    ...
    const response = await request(server).put('/v1/app/companies/images/logo').set('Authorization', login.body.access_token).send({
      invalid_field: true,
    });
  ...
  it('should return a status of bad request when not base 64 file type', async () => {
    await request(server).post('/v1/users').send({
      phone_country_code: phoneCountryCode,
      phone_area_code: phoneAreaCode,
      phone_number: phoneNumber,
    ...
    expect(Jimp.read).toHaveBeenCalledTimes(0);
    expect(mockWrite).toHaveBeenCalledTimes(0);
    expect(mockQuality).toHaveBeenCalledTimes(0);
    expect(mockWrite).toHaveBeenCalledTimes(0);
  });
  ...
  it('should return unauthorized when request without token', async () => {
    const response = await request(server).put('/v1/app/companies/images/logo').send();
