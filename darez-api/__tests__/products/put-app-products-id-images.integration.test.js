...
describe('PUT /v1/app/products/:ID/images', () => {
  let server;
  let connection;
  ...
  it('should upload a photo to a product that already has photos without resizing', async () => {
    ...
    const response = await request(server).put('/v1/app/products/12345/images')
  ...
  it('should upload a photo to a product that does not have photos and resize it', async () => {
    ...
    const response = await request(server).put('/v1/app/products/12346/images')
      .set('Authorization', login.body.access_token)
      .send({
        files: [
          {
  ...
  it('should return status of bad request when wrong base 64 file type', async () => {
    await request(server).post('/v1/users').send({
      phone_country_code: companyPhoneCountryCode,
      phone_area_code: companyPhoneAreaCode,
      phone_number: companyPhoneNumber,
    ...
    const response = await request(server).put('/v1/app/products/12345/images').set('Authorization', login.body.access_token).send({
      files: [
        {
    ...
    expect(Jimp.read).not.toBeCalled();
    expect(mockWrite).not.toBeCalled();
    expect(mockQuality).not.toBeCalled();
    expect(mockWrite).not.toBeCalled();
  });
  ...
  it('should return unauthorized when requested with unathorized token', async () => {
    ...
    const response = await request(server).put('/v1/app/products/12345/images').set('Authorization', login.body.access_token)
      .send();
