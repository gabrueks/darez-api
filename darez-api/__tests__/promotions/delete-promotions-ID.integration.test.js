...
describe('DELETE /v1/promotions', () => {
  ...
  beforeEach(async () => {
    ...
    await connection.promise().query(`INSERT INTO products
        ...
        VALUES
          (11111, 54321, 'product_promotion', 'some description', '10.00', 'CategoryTwo', 'SubCategoryTwo', 1, 123456, 9.00)`);
  });
  ...
  it('should delete a promotion', async () => {
    ...
    const [[infoBefore]] = await connection.promise().query('SELECT active FROM promotions WHERE id=123456');
    const [[infoProductBefore]] = await connection.promise().query('SELECT promotion, promotion_price FROM products WHERE id=11111');

    const response = await request(server).delete('/v1/promotions/123456')
      .set('Authorization', login.body.access_token);

    const [[infoAfter]] = await connection.promise().query('SELECT active FROM promotions WHERE id=123456');
    ...
    expect(infoBefore.active).toEqual(1);
    expect(infoProductAfter.promotion).toEqual(null);
    expect(infoProductAfter.promotion_price).toEqual(null);
    expect(infoAfter.active).toEqual(0);
    expect(infoProductBefore.promotion).toEqual(123456);
    expect(infoProductBefore.promotion_price).toEqual('9.00');
  });

  it('should return unauthorized when request without token', async () => {
    const response = await request(server).delete('/v1/promotions/123456').send();
