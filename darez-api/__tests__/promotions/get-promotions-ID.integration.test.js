...
describe('GET /v1/promotions/', () => {
  ...
  it('should get promotion by ID', async () => {
    const response = await request(server).get('/v1/promotions/123457').send();

    expect(response.status).toEqual(200);
    expect(response.body).toHaveProperty('promotion');

    expect(response.body.promotion.id).toEqual(123457);
    expect(response.body.promotion.company_id).toEqual(123987);
    expect(response.body.promotion.discount).toEqual('0.1200');
    expect(response.body.promotion.has_limit_date).toEqual(false);
    expect(response.body.promotion.date_start).toEqual('2020-10-01T00:00:00.000Z');
    expect(response.body.promotion.date_end).toEqual('2030-10-01T00:00:00.000Z');
    expect(response.body.promotion).toHaveProperty('products');

    expect(response.body.promotion.products[0].category).toEqual('CategoryTwo');
    expect(response.body.promotion.products[0].created_by).toEqual(null);
    expect(response.body.promotion.products[0].description).toEqual('Some description');
    expect(response.body.promotion.products[0].id).toEqual(123099);
    expect(response.body.promotion.products[0].name).toEqual('Product');
    expect(response.body.promotion.products[0].photo_key).toEqual([]);
    expect(response.body.promotion.products[0].price).toEqual('10.00');
    expect(response.body.promotion.products[0].promotion_price).toEqual('8.80');
    expect(response.body.promotion.products[0].subcategory).toEqual('SubCategoryTwo');
  });
});
