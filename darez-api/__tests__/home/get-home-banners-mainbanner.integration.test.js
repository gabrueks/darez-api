...
describe('GET /home/banners/mainbanner', () => {
  let server;
  let connection;
  ...
  it('should return the default home banner when no location is given', async () => {
    const response = await request(server).get('/v1/home/banners/mainbanner').send();
  ...
  it('should return the banner for a given locale in range', async () => {
    const response = await request(server).get('/v1/home/banners/mainbanner')
      .query({
        lat: '1.008',
        lng: '2.008',
      }).send();
