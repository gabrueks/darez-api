...
describe('GET /admin/orders/company/:ID', () => {
  let server;
  let connection;
  let login;
  ...
  it('shoud return unauthorized if no valid token is sent', async () => {
    const response = await request(server).get(`/v1/admin/orders/company/${testData.company_id}`).send();
  ...
  it('shoud return forbidden if user has no valid rights', async () => {
    ...
    const response = await request(server).get(`/v1/admin/orders/company/${testData.company_id}`)
      .set('Authorization', nonAdminLogin.body.access_token)// .query({ ID: testData.company_id })
      .send();
    expect(response.status).toEqual(401);
    expect(response.body).toEqual({ message: unauthorized });
  });

  it('shoud return all orders from requested company', async () => {
    const response = await request(server).get(`/v1/admin/orders/company/${testData.company_id}`)
