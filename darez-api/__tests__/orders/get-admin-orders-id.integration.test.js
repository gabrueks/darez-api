...
describe('GET /admin/orders/:ID', () => {
  ...
  it('shoud return unauthorized if no valid token is sent', async () => {
    const response = await request(server).get(`/v1/admin/orders/${testData.id}`).send();
  ...
  it('shoud return forbidden if user has no valid rights', async () => {
    ...
    const response = await request(server).get(`/v1/admin/orders/${testData.id}`)
  ...
  it('shoud return order data to admin', async () => {
    const response = await request(server).get(`/v1/admin/orders/${testData.id}`)
      .set('Authorization', login.body.access_token)
      .send();
